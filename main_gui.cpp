/**
 * @file main_gui.cpp
 * @brief Qt5 Modern Desktop GUI for CGPA Calculator (Refactored Spacing, Scroll View & Title Cleanup)
 * @details Premium dark-themed Qt application featuring full page scrolling, compact student & subject buttons, automatic 4 CH Theory/Lab split, Target CGPA simulator, and JSON/CSV profile persistence.
 */
...
struct Semester {
    ...
    void calculateGPA() {
        ...
        semesterGPA = (totalCredits > 0.0) ? (totalGradePoints / totalCredits) : 0.0;
    }
};
...
public:
    static std::pair<QString, double> getGradeFromMarks(double marks) {
        if (marks >= 90.00) return {"A",  4.00};
        if (marks >= 85.00) return {"A-", 3.70};
        if (marks >= 80.00) return {"B+", 3.30};
        if (marks >= 75.00) return {"B",  3.00};
        if (marks >= 70.00) return {"B-", 2.70};
        if (marks >= 65.00) return {"C+", 2.30};
        if (marks >= 60.00) return {"C",  2.00};
        if (marks >= 55.00) return {"C-", 1.70};
        if (marks >= 50.00) return {"D",  1.00};
        return {"F", 0.00};
    }

    static std::pair<QString, double> getGradeFromLetter(const QString& letter) {
        QString u = letter.trimmed().toUpper();
        if (u == "A" || u == "A+") return {"A", 4.00};
        if (u == "A-") return {"A-", 3.70};
        if (u == "B+") return {"B+", 3.30};
        if (u == "B") return {"B", 3.00};
        if (u == "B-") return {"B-", 2.70};
        if (u == "C+") return {"C+", 2.30};
        if (u == "C") return {"C", 2.00};
        if (u == "C-") return {"C-", 1.70};
        if (u == "D" || u == "D+") return {"D", 1.00};
        return {"F", 0.00};
    }

    static QString getRemark(double gradePoints) {
        if (gradePoints >= 4.00) return "Superior/Excellent";
        if (gradePoints >= 3.70) return "Excellent";
        if (gradePoints >= 3.30) return "Very Good";
        if (gradePoints >= 3.00) return "Above Average/Good";
        if (gradePoints >= 2.70) return "Good";
        if (gradePoints >= 2.30) return "Above Average";
        if (gradePoints >= 2.00) return "Competent Average";
        if (gradePoints >= 1.70) return "Below Average";
        if (gradePoints >= 1.00) return "Minimum Passing";
        return "Fail";
    }

    static QString getAcademicStanding(double cgpa) {
        if (cgpa >= 3.75) return "Summa Cum Laude (Highest Distinction / Rector's List)";
        if (cgpa >= 3.50) return "Magna Cum Laude (High Distinction / Dean's List)";
        if (cgpa >= 3.00) return "Cum Laude (Distinction / Good Standing)";
        if (cgpa >= 2.00) return "Satisfactory / Pass";
...
public:
    CGPACalculatorWindow(QWidget* parent = nullptr) : QMainWindow(parent) {
        ...
        if (screen) {
            ...
            int width = std::max(1050, static_cast<int>(screenGeometry.width() * 0.65));
            int height = std::max(750, static_cast<int>(screenGeometry.height() * 0.75));
            resize(width, height);
        } else {
            resize(1150, 800);
        }
...
private:
    ...
    void setupCalculatorTab(QWidget* tab) {
        ...
        // Student Info Group
        QGroupBox* infoGroup = new QGroupBox("👤 Student Profile & Entry Setup", scrollContent);
        ...
        fastModeGroup = new QGroupBox("⚡ Previous Academic Record (Fast Mode)", scrollContent);
        ...
        connect(modeComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int idx) {
            fastModeGroup->setVisible(idx == 1);
        });
        ...
        QGroupBox* courseGroup = new QGroupBox("📖 Add Course / Subject", scrollContent);
        ...
        connect(addCourseBtn, &QPushButton::clicked, this, &CGPACalculatorWindow::addCourse);
        ...
        coursesTable->setHorizontalHeaderLabels({"Course / Section Name", "Credit Hours", "Grade", "GPA Pts", "Earned Points", "Remark"});
        
        QHeaderView* header = coursesTable->horizontalHeader();
        header->setSectionResizeMode(0, QHeaderView::Stretch);           // Course Name gets space
        header->setSectionResizeMode(1, QHeaderView::ResizeToContents);  // Credit Hours
        ...
        header->setSectionResizeMode(4, QHeaderView::ResizeToContents);  // Earned Points
        ...
        QPushButton* calcBtn = new QPushButton("🧮 Calculate CGPA & Save Profile", this);
        calcBtn->setObjectName("CalcBtn");
        calcBtn->setCursor(Qt::PointingHandCursor);
        calcBtn->setFixedHeight(28);
        connect(calcBtn, &QPushButton::clicked, this, &CGPACalculatorWindow::calculateResults);
        ...
        connect(loadBtn, &QPushButton::clicked, [this]() {
            loadProfileFromJSON();
            QMessageBox::information(this, "Profile Loaded", "Saved profile loaded from 'student_record.json'.");
        });
    ...
    void setupSimulatorTab(QWidget* tab) {
        ...
        QGroupBox* simGroup = new QGroupBox("🎯 Target CGPA Goal & Degree Assumptions", scrollContent);
        QGridLayout* simLayout = new QGridLayout(simGroup);
        simLayout->setHorizontalSpacing(16);
        simLayout->setVerticalSpacing(12);

        simLayout->addWidget(new QLabel("Desired Target Graduating CGPA (0.00 - 4.00):"), 0, 0);
        ...
        simLayout->addWidget(new QLabel("Remaining Semesters to Graduate (1 - 8):"), 1, 0);
        ...
        simLayout->addWidget(new QLabel("Estimated Credit Hours per Remaining Semester:"), 2, 0);
        ...
        connect(runSimBtn, &QPushButton::clicked, this, &CGPACalculatorWindow::runSimulation);
        simLayout->addWidget(runSimBtn, 3, 0, 1, 2);

        layout->addWidget(simGroup);

        // Simulation Results Display Box
        QGroupBox* resultGroup = new QGroupBox("📊 Simulation Analysis & Strategic Plan", scrollContent);
        QVBoxLayout* resLayout = new QVBoxLayout(resultGroup);
        resLayout->setContentsMargins(14, 20, 14, 14);

        simulatorResultLabel = new QLabel("Run simulation to see required GPA and academic advice...", this);
        simulatorResultLabel->setStyleSheet("font-size: 14px; color: #f8fafc; padding: 8px;");
...
private slots:
    void addCourse() {
        ...
        if (name.isEmpty()) {
            QMessageBox::warning(this, "Input Error", "Please enter a valid course name.");
            return;
        }
        ...
        if (valToken.isEmpty()) {
            QMessageBox::warning(this, "Input Error", "Please enter percentage marks (0-100) or letter grade.");
            return;
        }
        ...
        if (ch == 4.0) {
            ...
            insertTableRow(name + " (Theory)", 3.0, gInfo.first, gInfo.second, (ok ? marksVal : -1.0));
            insertTableRow(name + " (Lab)", 1.0, gInfo.first, gInfo.second, (ok ? marksVal : -1.0));
        } else {
            bool ok;
            double marksVal = valToken.toDouble(&ok);
            std::pair<QString, double> gInfo;
            ...
            insertTableRow(name, ch, gInfo.first, gInfo.second, (ok ? marksVal : -1.0));
        }
    ...
    void insertTableRow(const QString& name, double ch, const QString& letter, double pts, double marks) {
        int row = coursesTable->rowCount();
        coursesTable->insertRow(row);

        coursesTable->setItem(row, 0, new QTableWidgetItem(name));
        coursesTable->setItem(row, 1, new QTableWidgetItem(QString::number(ch, 'f', 1)));
        coursesTable->setItem(row, 2, new QTableWidgetItem(letter));
        coursesTable->setItem(row, 3, new QTableWidgetItem(QString::number(pts, 'f', 2)));
        coursesTable->setItem(row, 4, new QTableWidgetItem(QString::number(pts * ch, 'f', 2)));
        coursesTable->setItem(row, 5, new QTableWidgetItem(ComsatsGradeEngine::getRemark(pts)));
    ...
    void calculateResults() {
        ...
        if (rows == 0 && modeComboBox->currentIndex() == 0) {
            QMessageBox::warning(this, "Calculation Error", "Please add at least one course to calculate.");
            return;
        }
        ...
        if (currentSemNum <= 8 && modeComboBox->currentIndex() == 0) {
            if (semCH < 12.0 || semCH > 21.0) {
                QMessageBox::warning(this, "COMSATS Policy Error", 
                    QString("Semester %1 total credit hours (%2 CH) violates COMSATS policy.\nSemesters 1-8 MUST have between 12.0 and 21.0 Credit Hours!").arg(currentSemNum).arg(semCH));
                return;
            }
        }
        ...
        cgpaLabel->setText(QString("Overall CGPA: %1 / 4.00").arg(QString::number(overallCGPA, 'f', 2)));
        totalCreditsLabel->setText(QString("Total Credits: %1 CH").arg(QString::number(totalCredits, 'f', 1)));
        standingLabel->setText(QString("Academic Standing: %1").arg(standing));

        saveProfileToJSON();
        QMessageBox::information(this, "Success", "CGPA calculated successfully! Profile persisted to 'student_record.json'.");
    }
    ...
    void runSimulation() {
        ...
        QString html = QString("<b>Current Cumulative CGPA:</b> %1 / 4.00 (%2 CH)<br>")
        ...
        if (reqGPA <= 0.0) {
            html += "<span style='color:#4ade80; font-size:16px;'>🎉 GREAT NEWS! You have already achieved or surpassed your target CGPA!</span>";
        } else if (reqGPA > 4.00) {
            double maxCGPA = (totalPoints + (4.00 * futureCH)) / finalCH;
            html += QString("<span style='color:#ef4444; font-size:16px;'>⚠️ MATHEMATICALLY IMPOSSIBLE:</span><br>")
                  + QString("To reach a %1 CGPA, you would need an average GPA of <b>%2</b> per semester.<br>")
                    .arg(targetCGPA).arg(QString::number(reqGPA, 'f', 2))
                  + QString("💡 Maximum achievable CGPA with perfect 4.00 in all remaining semesters: <b>%1 / 4.00</b>").arg(QString::number(maxCGPA, 'f', 2));
        } else {
            html += QString("<span style='color:#38bdf8; font-size:17px;'>📌 REQUIRED AVERAGE GPA: <b>%1 / 4.00</b> per remaining semester.</span><br><br>")
                    .arg(QString::number(reqGPA, 'f', 2));

            if (reqGPA >= 3.70) {
                html += "🔥 <b>STRATEGY:</b> You need mostly 'A' and 'A-' grades in all remaining courses.";
            } else if (reqGPA >= 3.30) {
                html += "👍 <b>STRATEGY:</b> You need mostly 'B+' and 'A-' grades in remaining courses.";
            } else {
                html += "✅ <b>STRATEGY:</b> Maintain steady 'B' / 'C+' performance to comfortably hit your target.";
            }
        }
    ...
    void saveProfileToJSON() {
        ...
        root["academicStanding"] = ComsatsGradeEngine::getAcademicStanding(overallCGPA);
        ...
        QFile file("student_record.json");
    ...
    void loadProfileFromJSON() {
        ...
        if (root.contains("courses") && root["courses"].isArray()) {
            ...
            for (const auto& val : arr) {
                ...
                insertTableRow(
                    cObj["courseCode"].toString(),
                    cObj["creditHours"].toDouble(),
                    cObj["letterGrade"].toString(),
                    cObj["gradePoints"].toDouble(),
                    -1.0
                );
            }
        }
